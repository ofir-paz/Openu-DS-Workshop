...
from src.first_train.base_model import BaseModel
...
class SpineCNN(BaseModel):
    ...
    def __init__(self, image_height: int, image_width: int, depth=32):
        ...
        self.conv2 = nn.Conv3d(in_channels=depth, out_channels=depth, kernel_size=(3, 3, 3), padding=1)
        self.conv3 = nn.Conv3d(in_channels=depth, out_channels=depth, kernel_size=(3, 3, 3), padding=1)
        ...
        self.fc1 = nn.Linear(conv_output_height * conv_output_width * depth, self.num_conditions * 3)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.sum(2) / x.size(2)  # Remove depth dimension.
        x = x.view(x.size(0), -1)
        x = self.dropout(x)
        x = self.fc1(x)
        return x
